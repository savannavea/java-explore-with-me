...
public interface StatsRepository extends JpaRepository<Stat, Long> {
    @Query(value = "SELECT NEW ru.practicum.statsDto.HitResponseDto(s.app, s.uri, COUNT(s.ip)) " +
            "FROM Stat AS s " +
    ...
    @Query(value = "SELECT NEW ru.practicum.statsDto.HitResponseDto(s.app, s.uri, COUNT(DISTINCT s.ip)) " +
            "FROM Stat AS s " +
            "WHERE s.uri IN :uris AND s.timestamp BETWEEN :start AND :end " +
